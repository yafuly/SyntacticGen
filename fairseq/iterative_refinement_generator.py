...
class IterativeRefinementGenerator(object):
    def __init__(
        ...
        self.models = models
        # ctc inference
        self.extra_symbols_to_ignore = []
        if hasattr(tgt_dict, "blank_index"):
            self.extra_symbols_to_ignore.append(tgt_dict.blank_index)
        if hasattr(tgt_dict, "mask_index"):
            self.extra_symbols_to_ignore.append(tgt_dict.mask_index)
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        def _ctc_postprocess(tokens):
            ...
            hyp = hyp.new_tensor([v for v in _toks if v not in self.extra_symbols_to_ignore])
            return hyp

        def finalized_hypos(step, prev_out_token, prev_out_score, prev_out_attn):
            cutoff = prev_out_token.ne(self.pad)
            tokens = prev_out_token[cutoff]
            tokens = _ctc_postprocess(tokens) # ctc post-process
