...
class LabelSmoothedDualImitationCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        for i in range(6):
            if f"glat_info_{i}" in outputs and outputs[f"glat_info_{i}"] is not None:
                # print(outputs[f"glat_info_{i}"])
                # logging_output[f'glat_context_p_{i}'] = outputs[f'glat_info_{i}']['glat_context_p'] 
                # logging_output[f"glat_accu_{i}"] = outputs[f'glat_info_{i}']['glat_accu']
                logging_output[f"glat_keep_{i}"] = outputs[f'glat_info_{i}']['glat_accu']

        for i in range(6):
            if f'word_ins_{i}' in outputs:
                if 'replace_ratio' in outputs[f'word_ins_{i}']:
                    logging_output[f'replace_ratio_{i}'] = outputs[f'word_ins_{i}'][f'replace_ratio']
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        for key in logging_outputs[0]:
            ...
            if "replace_ratio" in key:
                _l = list(log.get(key, 0) for log in logging_outputs)
                val = sum(log.get(key, 0) for log in logging_outputs)/len(_l)
                metrics.log_scalar(
                    key,
                    val,
                    round=3,
